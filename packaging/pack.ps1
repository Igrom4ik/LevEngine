...
+Packaging script for LevEngine (PowerShell)
...
if ($buildExit -ne 0)
{
    ...
    $candidates = @(
        ...
        (Join-Path $root "Test\LevEngine-*-Windows"),
        (Join-Path $root "Test\LevEngine-*-Windows\bin"),
        (Join-Path $root "Test\LevEngine-*-Windows\lib")
 )
...
# Create final zip
$zipName = "LevEngine-$Version-Windows-$Configuration.zip"
