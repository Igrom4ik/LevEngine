"""
LevEngine - Interactive Build Menu
...
class BuildMenu:
    ...
    def print_header(self):
        ...
        print(f"{Colors.BOLD}{Colors.OKCYAN}{'LevEngine - Build Automation Menu':^70}{Colors.ENDC}")
        print(f"{Colors.BOLD}{Colors.OKCYAN}{'=' * 70}{Colors.ENDC}\n")
    ...
    def action_generate_vs_solution(self):
        ...
        if success:
            # Попробуем найти любой .sln или .slnx файл в папке vs-build
            sln_dir = BuildConfig.PROJECT_ROOT / "vs-build"
            sln_candidates = []
            try:
                if sln_dir.exists():
                    sln_candidates = list(sln_dir.glob(f"{BuildConfig.get_project_name()}*.sln*"))
                    if not sln_candidates:
                        sln_candidates = list(sln_dir.glob("*.sln*"))
            except Exception:
                sln_candidates = []
            ...
            if sln_candidates:
                ...
                try:
                    ...
                    if open_vs == 'y':
                        # На Windows используем os.startfile чтобы не получать лишний вывод в консоль
                        if os.name == 'nt':
                            try:
                            ...
                            # For non-windows, attempt to open with xdg-open / open
                            import subprocess
                            try:
                                subprocess.Popen(["xdg-open", str(sln_path)])
                                print(f"{Colors.OKGREEN}✓ Opened solution{Colors.ENDC}")
                            except Exception:
                                try:
                                    subprocess.Popen(["open", str(sln_path)])
                                    print(f"{Colors.OKGREEN}✓ Opened solution{Colors.ENDC}")
                                except Exception as e:
                                    print(f"{Colors.WARNING}⚠ Не удалось открыть solution: {e}{Colors.ENDC}")
                except Exception as e:
                    print(f"{Colors.WARNING}⚠ Не удалось открыть VS: {e}{Colors.ENDC}")
            else:
                # Если не нашли .sln файлов, покажем стандартный путь (возможно другое расширение)
                sln_path = BuildConfig.PROJECT_ROOT / "vs-build" / f"{BuildConfig.get_project_name()}.sln"
                print(f"\n{Colors.OKGREEN}✓ Visual Studio solution сгенерирован (ожидаемый путь):{Colors.ENDC}")
                print(f"  Путь: {sln_path}")
        else:
             print(f"\n{Colors.FAIL}✗ Ошибка генерации Visual Studio solution{Colors.ENDC}")

    def run(self):
        """Main interactive loop for the build menu"""
        try:
            while self.running:
                try:
                    self.clear_screen()
                    ...
                    if choice == '1':
                        ...
                        # Tools menu could be a placeholder for future actions
                        # For now reuse build actions
                        self.menu_build_actions()
                    elif choice == '0' or choice.lower() == 'q' or choice == '':
                        self.running = False
                    else:
                        print(f"{Colors.FAIL}Invalid choice.{Colors.ENDC}")
                        input(f"\n{Colors.OKCYAN}Press Enter to continue...{Colors.ENDC}")
