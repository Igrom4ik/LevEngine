...
class BuildConfig:
    ...
    verbose = False
    clean_first = True
    # Optional override: allow user to force a specific CMake generator name
    USER_CMAKE_GENERATOR = None
    ...
    def get_cmake_generator():
        ...
        if BuildConfig.current_build_system == BuildSystem.NINJA:
            ...
            # Allow user to force generator via user config
            if getattr(BuildConfig, 'USER_CMAKE_GENERATOR', None):
                return BuildConfig.USER_CMAKE_GENERATOR

            # Try to detect installed Visual Studio instances by checking common paths
            vs_versions = [
                ("Visual Studio 18 2026", r"C:\\Program Files\\Microsoft Visual Studio\\2026"),
            ...
            # Fall back to vswhere detection if available
            try:
                vs_info = ToolPaths.find_visual_studio()
                if vs_info and 'version' in vs_info:
                    ver = str(vs_info['version'])
                    if '2026' in ver:
            ...
            # Default to the newest generator name
            return "Visual Studio 18 2026"
        elif BuildConfig.current_build_system == BuildSystem.MAKE:
            # On Windows prefer MinGW or NMake depending on toolchain
            if BuildConfig.current_toolchain == Toolchain.MINGW_GCC:
                return "MinGW Makefiles"
            if BuildConfig.current_toolchain in (Toolchain.MSVC, Toolchain.CLANG_CL):
...
class ToolPaths:
    ...
    VISUAL_STUDIO_PATHS = [
        # VS 2026 (новые пути)
        (r"C:\\Program Files\\Microsoft Visual Studio\\2026\\Community", "2026"),
        (r"C:\\Program Files\\Microsoft Visual Studio\\2026\\Professional", "2026"),
        (r"C:\\Program Files\\Microsoft Visual Studio\\2026\\Enterprise", "2026"),
        # VS 2022
    ...
    def find_visual_studio():
        ...
        # First try static known locations
        ...
        # If not found in common places, try vswhere (if available)
        try:
            vswhere_exec = shutil.which('vswhere')
            if not vswhere_exec:
                # Common location for vswhere
                possible_vswhere = Path(os.environ.get('ProgramFiles(x86)', '')) / 'Microsoft Visual Studio' / 'Installer' / 'vswhere.exe'
                if possible_vswhere.exists():
                    vswhere_exec = str(possible_vswhere)

            if vswhere_exec:
                # Query for latest installation that includes MSBuild component
                cmd = [vswhere_exec, '-latest', '-products', '*', '-requires', 'Microsoft.Component.MSBuild', '-property', 'installationPath']
                try:
                    out = subprocess.check_output(cmd, universal_newlines=True).strip()
                    if out:
                        vs_path = out.splitlines()[0].strip()
                        vc_tools = Path(vs_path) / "VC" / "Tools" / "MSVC"
                        vcvarsall = Path(vs_path) / "VC" / "Auxiliary" / "Build" / "vcvarsall.bat"
                        if vc_tools.exists() and vcvarsall.exists():
                            versions = sorted([d for d in vc_tools.iterdir() if d.is_dir()], reverse=True)
                            if versions:
                                msvc_version = versions[0]
                                compiler_path = msvc_version / "bin" / "Hostx64" / "x64" / "cl.exe"
    ...
    def initialize_tools():
        ...
        if not BuildConfig.CMAKE_PATH:
            ...
            print("  Suggestion: install CMake or set USER_CMAKE_PATH in Automation/user_config.py to the full path of cmake.exe")

        # Find Ninja
        BuildConfig.NINJA_PATH = ToolPaths.find_ninja()
        if not BuildConfig.NINJA_PATH:
            print("⚠ Ninja not found in the system!")
            print("  (Optional) Suggestion: install Ninja or set USER_NINJA_PATH in Automation/user_config.py")
        ...
        if vs_info:
            ...
            print("  Suggestion: install Visual Studio (Desktop development with C++) or set USER_VCVARSALL_PATH in Automation/user_config.py")
