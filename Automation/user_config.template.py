...
# Принудительный генератор CMake (если нужно переопределить, например при новой версии Visual Studio)
USER_CMAKE_GENERATOR = None  # Например: "Visual Studio 18 2026"
...
def apply_user_config(build_config):
    ...
    # Применяем пользовательский генератор CMake если задан
    if 'USER_CMAKE_GENERATOR' in globals() and USER_CMAKE_GENERATOR:
        setattr(build_config, 'USER_CMAKE_GENERATOR', USER_CMAKE_GENERATOR)
